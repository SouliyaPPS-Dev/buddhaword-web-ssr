    ...
    if (seApiKey) {
        ...
        seChain.then(function() {
            ...
            seBuffers.forEach(function(buf) {
                decChain = decChain.then(function() {
                    return new Promise(function(resolve) {
                        try { ttsState.audioCtx.decodeAudioData(buf, function(b) { decoded.push(b); resolve(); }, function() { resolve(); }); }
                        catch(e) { resolve(); }
                    });
                });
            });
            decChain.then(function() {
                ...
                var merged = ttsState.audioCtx.createBuffer(decoded[0].numberOfChannels, totalLen2, decoded[0].sampleRate);
                var offset = 0;
