...
function toggleTTS() {
    ...
    // 1. Try server API (provides word-accurate timepoints)
    ...
    // 3. Try StreamElements directly from browser (only if API key configured)
    ...
    if (seApiKey) {
        ...
        seChain.then(function() {
            ...
            seBuffers.forEach(function(buf) {
                decChain = decChain.then(function() {
                    return new Promise(function(resolve) {
                        try { ttsAudioCtx.decodeAudioData(buf, function(b) { decoded.push(b); resolve(); }, function() { resolve(); }); }
                        catch(e) { resolve(); }
                    });
                });
            });
            decChain.then(function() {
                ...
                var merged = ttsAudioCtx.createBuffer(decoded[0].numberOfChannels, totalLen2, decoded[0].sampleRate);
                var offset = 0;
                ...
                doPlay(merged, estimateTimepoints(merged.duration));
            });
        })
        .catch(function(e) { console.warn('StreamElements error:', e); });
    }
}
